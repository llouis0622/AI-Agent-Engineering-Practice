...
def evaluate_single_instance(raw: str, graph) -> Optional[Dict[str, float]]:
    ...
    try:
        ...
        if "input" in ex and "expected_function_call" in ex:
            ...
            elif customer_id or account_id or expected_call["name"] in ["investigate_transaction", "freeze_account",
        ...
        for msg in result["messages"]:
            if isinstance(msg, AIMessage) and msg.content.strip().lower() in ["inventory", "transportation",
                                                                              "supplier"]:
